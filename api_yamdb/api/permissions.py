...
class IsAuthor(permissions.BasePermission):
    def has_permission(self, request, view):
        if request.user.is_anonymous is True:
            return False
        return request.user.role == request.user.is_user() or request.user.is_superuser
...
class IsModerator(permissions.BasePermission):
    def has_permission(self, request, view):
        if request.user.is_anonymous is True:
            return False
        return (
            request.user.role == request.user.is_moderator()
...
class IsAdmin(permissions.BasePermission):
    def has_permission(self, request, view):
        if request.user.is_anonymous is True:
            return False
        return request.user.role == request.user.is_admin() or request.user.is_superuser
